...
# ──────────────────────────────────────────────
# Native FPS + OpenCV text-change detector
# ──────────────────────────────────────────────

def _ffprobe_bin() -> str:
    ffmpeg = Path(_cfg["ffmpeg_bin"])
...
def _probe_native_fps(video_path: Path) -> float | None:
    ...
    try:
        ...
        _cfg["log"](f"Step1 PaddleOCR: native FPS probe failed: {exc}")
    return None


def _resolve_extract_fps(video_path: Path) -> tuple[float, float | None]:
    """target scan fps clamped to native; returns (extract_fps, native_fps|None)."""
    target = float(_cfg["fps"])
    native = _probe_native_fps(video_path)
    ...
    if target < 5.0:
        _cfg["log"](
            f"Step1 PaddleOCR: WARNING scan_fps={target:.2f} < 5 — "
...
def _build_text_mask(img):
    """Binary text mask on preprocessed crop strip (OpenCV nhẹ)."""
    import cv2
    import numpy as np
    ...
    if wt > 0:
        ...
        # Preprocess often yields bright text on dark; Otsu + invert if needed
...
def _is_blank_mask(mask, min_ink_ratio: float = 0.0015) -> bool:
    import numpy as np
...
def _mask_change_score(mask_a, mask_b) -> float:
    """MAD on 0/255 masks (0–255). ~8 ≈ vài % pixel đổi."""
    import numpy as np
...
def _build_cues_from_opencv(
    ...
    Returns (cues, debug_rows).
    Each cue: {start, end, ocr_idx, ocr_path, select_reason}
    """
    import cv2

    threshold = float(_cfg["framediff_threshold"])
    log = _cfg["log"]
    cues: list[dict] = []
    debug_rows: list[dict] = []

    state: dict | None = None  # start, last_text_t, key_mask, ocr_idx, ocr_path
    ...
    for idx, fpath in enumerate(
        _cfg["progressbar"](frame_files, total=total, desc="OpenCV text-change")
    ):
        ...
        mask = _build_text_mask(img)
        blank = _is_blank_mask(mask)
        row["is_blank"] = blank
        ...
        if state is None:
            ...
            score = _mask_change_score(mask, state["key_mask"])
            row["opencv_change_score"] = score
            if score >= threshold:
                # change: close old at last_text_t, open new at t
                _close_state()
                state = {
                    "start": ts,
                    "last_text_t": ts,
        ...
        if done == 1 or done % log_every == 0 or done == total:
            log(
                f"Step1 PaddleOCR: OpenCV progress {done}/{total} "
...
def _apply_noise_filter(cues: list) -> list:
    ...
    if dropped:
        log(f"Step1 PaddleOCR: noise filter dropped {dropped} short cue(s) (<{noise_ms}ms)")

    # B. Merge consecutive duplicates
    if not kept:
        return []
    merged = [list(kept[0])]
    merges = 0
    for start, end, text in kept[1:]:
        if same_subtitle_line(merged[-1][2], text, fuzzy_thr):
            merged[-1][1] = end
    ...
    if merges:
        log(f"Step1 PaddleOCR: noise filter merged {merges} consecutive duplicate cue(s)")
...
def _ocr_with_paddleocr(video_path: Path) -> Path:
    ...
    extract_fps, native_fps = _resolve_extract_fps(video_path)
    ...
    cue_shells, opencv_debug = _build_cues_from_opencv(frame_files, frame_interval_sec)
    if not cue_shells:
        raise RuntimeError("Step1 PaddleOCR: OpenCV found no subtitle cues.")

    # Mark OCR frames in opencv debug
    ...
    if low_conf_by_idx and bridge_min > 0:
        all_cands = sorted(
            [(ts, tx) for ts, tx in ocr_by_idx.values()]
    ...
    # Attach OCR text to cue shells (timeline already has correct start/end + gaps)
    raw_cues: list = []
    ...
    # Debug jsonl: opencv rows + OCR rows
    debug_path = ocr_dir / "frame_ocr_raw.jsonl"
    with open(debug_path, "w", encoding="utf8") as _df:
        for row in opencv_debug:
        ...
        for row in sorted(ocr_debug_rows, key=lambda r: r["frame_index"]):
            ...
            row["kind"] = "paddleocr"
            _df.write(json.dumps(row, ensure_ascii=False) + "\n")
    log(f"Step1 PaddleOCR: debug log → {debug_path}")

    if not raw_cues:
        raise RuntimeError("Step1 PaddleOCR: no text survived cleaning.")

    # Fuzzy merge across small gaps (same text)
    ...
    # Noise filter (drop short + consecutive duplicate merge)
    kept = _apply_noise_filter(kept)
    if not kept:
        raise RuntimeError("Step1 PaddleOCR: all cues removed by noise filter.")

    # Optional pad (default 0 = off)
    min_dur = int(_cfg.get("min_duration_ms") or 0)
