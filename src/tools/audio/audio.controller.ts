...
export class AudioController {
  ...
  @ApiOperation({ summary: "List user audio generation jobs" })
  @ApiQuery({ name: "userId", required: true })
  @Public()
  @Get("jobs")
  async listJobs(@Query("userId") userId?: string) {
    if (!userId) {
      throw new BadRequestException("userId is required");
    }
    const rows = await this.audioService.getHistory(userId);
    return rows.map((row) => this.audioService.mapHistoryForClient(row));
  }
