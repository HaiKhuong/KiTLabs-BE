...
export class DouyinController {
  ...
  async extract(@Body() dto: ExtractDouyinDto) {
    return this.douyinService.extractVideo(dto.url, dto.cookieContent);
  }
  ...
  async extractProfile(@Body() dto: ExtractProfileDto) {
    return this.douyinService.extractProfile(dto.url, dto.cookieContent, dto.maxVideos);
  }
  ...
  async download(@Body() dto: DownloadDouyinDto, @Res() res: Response) {
    try {
      const streamResponse = await this.douyinService.downloadVideo(dto.url, dto.formatId, dto.cookieContent);
